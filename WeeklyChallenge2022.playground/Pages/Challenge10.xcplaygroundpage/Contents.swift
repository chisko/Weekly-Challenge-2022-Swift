...
func equilibrada(expresion: String) -> Bool {
    var result: [Int] = []
    
    for char in expresion {
        if let openIndex = ["[","{","("].firstIndex(of: String(char)) {
            result.append(openIndex)
        } else if let closeIndex = ["]","}",")"].firstIndex(of: String(char)) {
            if let lastIndex = result.last, lastIndex == closeIndex {
                result.popLast()
            } else {
                return false
            }
